...
```{r}
setwd("~/Documents/Bioinformatics/SmartIdea/FunEA.analysis/")
```

```{r}
library(tidyverse)

# Read all files with extension .TPM_counts.txt
file_list <- list.files(pattern = "\\.TPM_counts\\.txt$")

annotations.filtered <- readRDS("ko.annotations.rds")
...
for (file in file_list) {
  ...
  # Read the file into a data frame, check for empty files
  tpm_data <- tryCatch(
  ...
  if (!is.null(tpm_data)) {
    # Ensure SampleID column is not missing
    if (!"SampleID" %in% colnames(tpm_data)) {
      message("Warning: 'SampleID' column missing in ", file)
      next
    }
    ...
    tpm_data.annotated <- left_join(tpm_data, annotations.filtered, by = c("GeneID" = "query"))

    tpm_data.annotated.summarised <- tpm_data.annotated %>% group_by(KEGG, SampleID) %>% summarise(TPM = sum(TPM))   
    
    # Append the data frame to the list
    tpm_data_list[[file]] <- tpm_data.annotated.summarised
  }
}

# Combine all data frames into one
combined_tpm_data <- bind_rows(tpm_data_list)

# Pivot the data wider, making each SampleID a column
wide_tpm_data <- combined_tpm_data %>%
  pivot_wider(
    names_from = SampleID,
    values_from = TPM,
    values_fill = list(TPM = 0)  # Fill missing values with 0
  )
...
write.csv(wide_tpm_data, "Combined_TPM_Wide.csv", row.names = FALSE)
```
